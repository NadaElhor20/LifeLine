    ...
    email: {
      ...
      validate: {
        validator: function (email) {
          ...
          return BloodBank.findOne({ email }).then((BloodBank) => !BloodBank); // Returns true if email is unique
        },
...
bloodBankSchema.methods.checkPassword = function (plainPassword) {
  return bcrypt.compare(plainPassword, this.password);
};

bloodBankSchema.methods.generateToken = function () {
  return signInToken({ id: this.id }, privateKey, { expiresIn: '24h' });
};

bloodBankSchema.statics.getEntityFromToken = async function (token) {
